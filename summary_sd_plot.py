...
# Output: heatmap plot of the fraction of cooperators as a function of pF and r, for different M
    ...
    nr=3
    nc=4
    f,axs=plt.subplots(nrows=nr, ncols=nc, sharex='all', sharey='all', figsize=(13,10))
    ...
    titles = ["1 bit", "2 bits strength", "2 bits leader"]
    ...
    for idx in range(len(rv)):
        ...
        h=ax.contourf(MAT[:,:,rv[idx]-1,8],levels,cmap='winter', origin='lower', alpha=0.4, extend="max")
        h=ax.contourf(MAT[:,:,rv[idx]-1,0],levels,cmap='summer', origin='lower', alpha=0.5, extend="max")
        ...
        ax.set_yticklabels(np.linspace(deltaFv[0],deltaFv[-1],nticksY), fontsize=18)
        ax.set_ylim(0,MAT.shape[1]-1)
        if idx == 0: ax.set_title(f"4 bits", fontsize=fntsize)

    labels = ['ALLD', 'WDSC/NDLC', 'ALLC']
    cmaps=['Greens','Blues','Purples']
    patches = [mpatches.Patch(color="none", label="1/2 bits:"),]
    ...
    patches += [mpatches.Patch(color="none", label="4 bits:"),]
    labels = ['***0', '***1']
    cmaps=['Greens','Blues']
    patches += [mpatches.Patch(color=plt.get_cmap(cmaps[i])(0.9), label=labels[i]) for i in range(2)]
    plt.legend(handles=patches, loc='upper center', bbox_to_anchor=(-1.4 , -0.5),
          fancybox=True, shadow=False, ncol=7, fontsize=20, columnspacing=1.0)
...
#cb=f.colorbar(h, fraction=0.1,format='%.2f')
    ...
    f.savefig('./newtests/summary_multi_sd_4bits.png',bbox_inches='tight',dpi=300)
    #plt.show()
    f.clf()     
    return
